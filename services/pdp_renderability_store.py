...
═══════════════════════════════════════════════════════════════════════════════
A stored composite is only as good as its invalidation, and this one needs TWO
triggers:

  (1) the row or its seed changing        — **NOT WIRED**
  (2) serving_eligible flipping for the
      row's content_key                   — wired, see refresh_for_content_key

**44 distinct code paths INSERT/UPDATE catalog_products.** Wiring (1) across all
of them confidently was not possible in one pass, and wiring it *partially* is
precisely how ``catalog_row_trust`` and the minted/mirror split became stale
derivatives. Here it would fail in the most expensive direction: advertising a
URL that has stopped rendering.
...
# The new columns are declared as a LIGHTWEIGHT table() here, NOT added to
# db.catalog.catalog_products.
#
# This is a deploy-order safeguard, not a style choice. Adding a column to the
# shared Core Table makes EVERY `select(catalog_products)` in the codebase emit
# it, so the instant this code deploys against a database that has not yet run
# migration 188 every one of those SELECTs is an UndefinedColumn 500 — the exact
# failure recorded in this repo's history as "SQLAlchemy metadata column breaks
# SELECTs, ADD COLUMN via psql AT MERGE TIME". A local construct means nothing
# references the column except the code that deliberately asks for it, so the
# migration and the deploy are independent and neither can break the other.
_persisted = sa.table(
    "catalog_products",
    sa.column("product_key"),
    sa.column(COLUMN_WILL_RENDER, sa.Boolean),
    sa.column(COLUMN_COMPUTED_AT),
)
    ...
    for row in rows or []:
        ...
        await database.execute(
            sa.text(
                ...
                f"SET {COLUMN_WILL_RENDER} = :wr, {COLUMN_COMPUTED_AT} = NOW() "
                f"WHERE product_key = :pk"
            ),
            {"wr": bool(mapping.get("will_render")), "pk": mapping.get("product_key")},
        )
        written += 1
    return written
