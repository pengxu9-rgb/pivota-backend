...
class CanonicalPdpEnrichmentAgent(BaseExecutorAgent):
    ...
    async def should_run(self, context: ExecutorContext) -> bool:
        """Run when merchant_id is set, a Gemini key is configured, and the
        merchant has >=1 content-thin canonical PDP not yet enriched. Cheap —
    ...
    async def execute(self, context: ExecutorContext) -> ExecutorResult:
        ...
        candidates = await _fetch_thin_canonical_pdps(
            context.merchant_id, cap=_MAX_ENRICH_PER_RUN
        )
        if not candidates:
            return ExecutorResult(
                status="skipped",
