...
def _run_mentions_merchant(
    ...
    try:
        ...
        facts = compute_run_facts(
            [run],
            # This IS a comparison run — the internal rollup is the one caller
            # that must see it, so opt back in past the default exclusion.
            include_internal_comparison=True,
            merchant_host=None,
