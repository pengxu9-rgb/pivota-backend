...
def _mint_submission_token(
    *,
    ...
    jti = _b64u_encode(os.urandom(18))
...
def _verify_hmac_token(token: str, *, secret: bytes) -> VerifiedSubmission:
    if not token:
        raise HTTPException(status_code=401, detail="UNAUTHORIZED")
    try:
...
def verify_proof_token(token: str) -> VerifiedSubmission:
    # Proof token is minted by an upstream system (multi-platform), exchanged for a submission token.
    return _verify_hmac_token(token, secret=_proof_signing_secret())


async def exchange_proof_for_submission_token(
    *,
    request: Request,
    proof_token: str,
    ttl_seconds: int,
    ...
    # Single-use proof enforcement: store jti hash so proof cannot be exchanged repeatedly.
    await _consume_token_jti(merchant_id=verified.merchant_id, jti_hash=verified.jti_hash, exp=verified.exp)

    # Never mint a submission token that outlives the proof.
    now = _now_ts()
