...
from sqlalchemy import String, and_, cast, column, exists, func, literal, or_, select, table
...
def _pg_typed(value: str):
    """A string literal Postgres can infer a type for.

    THIS IS NOT DECORATION — it is the fix for a prod outage.

    The first cut of this module passed a bare Python string to ``func.concat``.
    SQLAlchemy binds that as an untyped parameter, and ``concat`` is declared
    ``VARIADIC "any"`` — there is no argument type for Postgres to infer from,
    so ``concat($1, col)`` is indeterminate. Postgres answers with
    ``could not determine data type of parameter`` (asyncpg:
    ``IndeterminateDatatypeError``), which took ``GET /api/canonical/products``
    to a hard 500 for every caller on 2026-07-26 (#1588, reverted in #1590).

    SQLite has no such inference rule, so the entire suite stayed green
    throughout. An explicit ``CAST(... AS VARCHAR)`` gives Postgres the type and
    is a no-op on SQLite, so both engines keep working.

    Scope note: the JSON ``->`` path reads were the other suspect, since ``->``
    is overloaded on ``jsonb -> int`` and ``jsonb -> text``. They are
    deliberately NOT wrapped — see :func:`_has_dossier`, where casting them is
    shown to be both unnecessary and actively harmful.

    ``test_content_depth_emits_no_indeterminate_params`` pins this.
    """
    return cast(literal(value), String)
...
def _has_dossier(cp):
    ...
    kb_keys = [
        and_(
            ...
            aurora_product_intel_kb.c.kb_key == func.concat(_pg_typed(_KB_KEY_PREFIX), col),
        )
        for col in (
    ...
    # DELIBERATELY NOT wrapped in _pg_typed, though `->` is overloaded and was
    # the other suspect for the #1588 outage. Two reasons, in order:
    #
    #   1. SQLAlchemy already binds these with a concrete `JSONStrIndexType`,
    #      not an untyped literal — unlike the bare Python string that reached
    #      `concat`. The error Postgres actually raised was
    #      "could not determine data type of parameter", which is the
    #      VARIADIC "any" signature specifically.
    #   2. Casting them is not merely unnecessary, it is WRONG: a
    #      `CAST(... AS VARCHAR)` index stops SQLAlchemy building the dialect's
    #      JSON path, and SQLite fails outright with "bad JSON path". That
    #      would take out the engine every executable test in this module runs
    #      on, trading a Postgres bug for a total loss of coverage.
    core_present = or_(
