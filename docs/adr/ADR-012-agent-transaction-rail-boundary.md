...
**Source of record — reconciled (was a transitional divergence).** The early did:key/did:web slices overloaded `agents.public_key` to hold the DID string, which diverged from this decision (a DID conflated with a key column, the exact thing the model separates). This is now aligned: `ConsentService.get_agent_identity(agent_id)` reads **`agents.did` first** (the durable home per this decision), and the DID's key is resolved from it via `resolve_agent_identity`. It **falls back** to a DID-or-PEM in `agents.public_key` so every already-provisioned agent keeps working during the transition. All AP2 identity readers (grant, `verify_ap2_signature`, transaction/mandate wiring) go through this seam. `agents.public_key` reverts to what its name means — a raw-PEM legacy/pilot key, no longer an identity carrier.
...
**Trust scope — decided (#1495): a platform-GLOBAL tier + per-agent bindings.** This is what lets external agents transact *at scale* without per-agent approval. A **global** trusted issuer (stored under a sentinel agent id, `db/ap2_trusted_issuers.py`) is honored for **any** agent's mandate — so a frontier / app platform's issuer DID is enrolled **once** and covers its whole agent fleet ("approve issuers, not agents"). **Per-agent** rows remain for the user-as-issuer / personal-agent case (a user's own DID authorizing their own agent). `get_trusted_issuers(agent_id)` returns the **union** (per-agent ∪ global), so the verify primitive and transaction route are untouched. Enrollment (previously the "highest-risk unspecified piece") is now an **admin** surface with **DID-resolution proof-of-control** — `POST /admin/ap2/trusted-issuers` resolves the issuer DID before trusting it (a `did:web` is honored only if its domain serves a `.well-known/did.json` binding that DID; a `did:key` is self-certifying), with immediate revocation. A canonical account id (to move per-agent bindings to account level) and self-serve issuer enrollment remain later slices.
...
6. [~] **VC / mandate authority layer** — verification primitive + constraint enforcement (#1455), Intent→Cart→Payment chain linkage (#1460), trusted-issuer registry (#1461), and transaction-flow wiring (#1462) are **done**. Trusted-issuer **scope + enrollment** decided and shipped (#1495): a platform-**global** tier + per-agent bindings, with an admin enrollment endpoint (`POST /admin/ap2/trusted-issuers`) gated by DID-resolution proof-of-control + revocation (see "Trust scope" above). **Remaining:** a canonical account id (to lift per-agent bindings to account level) and self-serve issuer enrollment. Design in "VC / mandate authority layer" above.
7. [ ] **SSRF hardening for self-registration** — if agents self-register `did:web`, pin the resolved IP into the connection (close the check-then-fetch TOCTOU gap noted in `ap2_did_web.py`). NB: the #1495 admin enrollment endpoint already resolves an admin-supplied `did:web` for proof-of-control — bounded (admin-initiated, and the existing SSRF guard blocks private/loopback/link-local IPs), so the residual TOCTOU is low there; the IP-pin is still required before **self-serve** issuer/agent `did:web` registration.
