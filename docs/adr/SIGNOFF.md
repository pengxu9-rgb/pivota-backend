...
- **The bet is attribution, not payments.** ADR-016 *removes* the custody/regulatory track; in exchange, Pivota's revenue depends on proving it drove a sale from *outside* the fund flow (ADR-017). That is the real thing you're signing up for.

---

## The load-bearing decision — ADR-016: Pivota is non-custodial

- **Decision:** Pivota never touches money. It brokers trust (identity / consent / mandate + a **signed receipt**), provides settlement **routing**, and records transactions for **attribution**. Money settles agent↔merchant on their own rails. Value = attribution/referral (merchant-side) + identity/authorization/receipt brokering (agent/protocol-side) — **never payment fees**.
- **What "yes" buys:** deletes the money-transmitter / custody / reserves regulatory track *and* the ledger/custodian build; keeps the moat on information + trust; one money boundary across the ACP and AP2 rails.
...
|---|---|---|---|
| **016** Non-custodial | Pivota never touches money; brokers trust + attribution | **Founder** | Proposed |
