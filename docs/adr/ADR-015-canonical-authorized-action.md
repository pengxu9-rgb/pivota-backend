...
Authorization-layer; the gate is **additive and strictly tightening** — today's paths are *less* strict (the consent path authorizes on token validity alone), so introducing the gate only denies what was already wrongly allowed. Reversing means reverting to per-protocol enforcement (Option B) — but note the consent-path gap (item 3) is a **live correctness issue** this ADR fixes regardless of the broader framework decision.
