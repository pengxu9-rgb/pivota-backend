...
- Last updated: 2026-05-08 (UTC) — **Onboarding standardization track at O-3 ✅**. O-1 + O-1 followup + O-2 + O-3a (LabelAgent service) + O-3b (batch worker) all merged. The full pipeline is in place — deterministic taxonomy v1 fires at every ingest, LabelAgent worker fills the long tail when invoked. Worker doesn't auto-run; first prod usage = O-6 canonical-only backfill. Next: O-4 (lifecycle stage column + promotion logic) OR O-6 (try the worker on the small canonical-only set first to validate end-to-end before adding more layers).
- Owner: peng
