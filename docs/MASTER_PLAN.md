...
- Last updated: 2026-05-08 (UTC) — **O-4 PR #387 open**: pdp_lifecycle_stage column + DRAFT→CANDIDATE→VALIDATED→PUBLISHED gates + 3-path wiring. mig 077 + partial index on live stages + `services/pdp_lifecycle.py` + Path A/B/C all compute and persist stage at write. 138 affected tests green. Next after merge + prod verify: O-6b backfill existing rows (else O-5's recall filter would NULL-out the candidate set), then O-5 recall live-stage filter.
- Owner: peng
