...
| # | Item | Effort | Where | Why |
|---|---|---|---|---|
| 7 | **Widen `OUTBOUND_WARM_HANDOFF_BRANDS`** from 6 to the Tier-1 cohort. **Read [docs/runbooks/outbound_warm_handoff_rollout.md](./runbooks/outbound_warm_handoff_rollout.md) first** — widening makes the `cart_prefilled: false` execution-spec field (#1822) a false negative for the added brands, and retroactively falsifies `false` answers on redirect tokens minted up to 7 days earlier. | S | Env var | The cart-prefill path is already proven; it is an allowlist edit plus monitoring — plus the `cart_prefilled` conflict, which must be resolved before or with the widening. |
