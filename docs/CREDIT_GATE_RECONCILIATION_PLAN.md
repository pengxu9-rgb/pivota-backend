...
**Status:** Ready to execute · backend CI unavailable until next month (Actions billing) → Phase 1/2 land then, or admin-merge with local tests · **Date:** 2026-06-16
**Implements:** [ADR-005](adr/ADR-005-audit-access-and-credits-model.md). **Corrects** ADR-005's "Reconciliation appendix" (mis-sized the work) and a since-resolved decision: **paid-tier overage is intended design — kept, not killed** (D1 resolved; Phase 3 dropped). The only open decision is D2 (canonical ledger, Track B).
...
2. ~~The balance gate only applies to free tier.~~ **NOT a gap — intended.** Paid tiers overspend `balance` into metered **overage** (`merchant_credit_balance_service.py:724-731`; `audit_runs_routes.py:1029,1044`) by design. Free tier is hard-gated; paid tier = balance + overage. Keep as-is (ADR-005 §1 amended to say so). No Phase 3.
...
- **D1 — Paid-tier overage — RESOLVED: KEEP (it's intended design).** Paid tiers are *meant* to overspend their balance into metered overage and settle via overage billing. So the "balance only hard-gates free tier; paid tiers accrue overage" behavior (#2 below) is **correct, not a divergence** — there is no Phase 3, and the overage subsystem stays. ADR-005 §1 has been amended to state the free=hard-gate / paid=balance+overage rule explicitly.
...
### ~~Phase 3 — Balance gate applies to paid tiers~~ — DROPPED
Paid-tier overage is intended design (D1 resolved: keep). The `paid_tier` bypass in
`_apply_delta` + the `not paid_tier`-gated 402s are the **correct** free=hard-gate /
paid=balance+overage behavior — leave them. Only ADR-005's wording needed amending
(done), no code change.
...
1. **Backend CI is unavailable until next month** (the GitHub Actions billing/spending-limit only clears then). So Phase 1/2 backend changes either wait for that, or land via **admin-merge with local pytest** (the path #908/#910/#911 already used — CI checks aren't hard-required). Portal pieces (the top-up CTA) are Vercel-CI and unaffected.
2. **Run the Track-B data check** any time — it's read-only (no CI needed) and resizes the biggest unknown.
3. **Phase 1** → **Phase 2** (high-value, low-risk, mostly subtractive + one button).
4. **Decide D2**, then **Track B** as its own effort.

**Net:** the visible ADR-005 surface (premium no longer needs a plan; top-up reachable; honest copy; URL=3) is reachable in **Phases 1-2 — small, low-risk** — because the credit plumbing already exists. The only genuinely hard, decision-gated item left is the **Track-B two-ledger consolidation** (D2). Paid-tier overage stays as designed.
