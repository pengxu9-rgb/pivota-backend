...
- Ops can query transaction-safety counters through `GET /orders/ops/transaction-safety/metrics`, including paid merchant-order failures, retry success/failure, quote revalidation failure, reconciliation drift, webhook duplicate, webhook failure, and fallback pollution attempt counts. Raw `webhook_failed_count` is retained for investigation, while `webhook_failed_order_impacting_count` is the page-level commerce-safety signal. The metrics response now separates active unresolved risk (`paid_merchant_order_failed_active_count`) from historical retry event counts (`merchant_order_retry_failed_event_count`) so old retry failures do not look like current paid-without-merchant-order exposure.
