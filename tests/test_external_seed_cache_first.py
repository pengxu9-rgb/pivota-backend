...
async def test_external_seed_cache_miss_empty_beauty_result_does_not_cache(
    ...
    live_loader.assert_awaited()


@pytest.mark.asyncio
async def test_external_seed_empty_cache_refreshes_for_unified_relevance(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import routes.agent_api as agent_api_module
    ...
    agent_api_module._put_cached_external_seed_products(cache_key, [])

    refreshed_item = {
        "id": "ext_refresh_lip_1",
        "product_id": "ext_refresh_lip_1",
        "merchant_id": "external_seed",
        "source": "external_seed",
    ...
    assert len(products) == 1
    assert products[0]["product_id"] == "ext_refresh_lip_1"
    assert metrics.get("executed") is True
    assert metrics.get("skip_reason") == "cache_miss_sync_filled"
    assert metrics.get("cache_hit") is False
    cached = agent_api_module._get_cached_external_seed_products(cache_key)
    assert isinstance(cached, list)
    assert len(cached) == 1
    assert cached[0]["product_id"] == "ext_refresh_lip_1"
    live_loader.assert_awaited()
