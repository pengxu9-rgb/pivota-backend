...
async def test_checkout_webhook_enqueues_the_merchant_order_durably(
    ...
    async def fake_get_primary_store(merchant_id: str):
        return {"platform": "shopify", "domain": "x.myshopify.com"}

    # payment_routes imports this INSIDE the handler, so patch the source module.
    monkeypatch.setattr(merchant_store_service, "get_primary_store", fake_get_primary_store)
...
@pytest.mark.asyncio
async def test_checkout_webhook_success_branch_respects_the_store_guard(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The two sites in this webhook used to dispatch the merchant-order create
    through BackgroundTasks, which died with the process. They now enqueue, and
    ...
    async def fake_get_primary_store(merchant_id: str):
        return {"platform": "woocommerce", "domain": "shop.example"}
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.post(
            "/api/payments/webhooks/checkout",
            json=_checkout_payload("ORD_ENQUEUE_WOO", event_id="evt_enqueue_woo"),
        )

    assert resp.status_code == 200
    # The SUCCESS branch, distinct from the already-paid one: this webhook has
    # only ever created merchant orders for a Shopify PRIMARY store.
...
@pytest.mark.asyncio
async def test_checkout_webhook_does_not_enqueue_for_a_non_shopify_primary(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """This webhook has only ever created merchant orders for a Shopify PRIMARY
    store, even though `sync_order_to_connected_store` also dispatches
    WooCommerce, BigCommerce and Wix. Enqueuing regardless would silently start
    creating orders on platforms it has never served."""
    import db.orders as orders_module
    import routes.payment_routes as payment_routes_module
    import services.merchant_store_service as merchant_store_service
    import services.webhook_service as webhook_module
    ...
    async def fake_get_order(order_id: str) -> Dict[str, Any] | None:
        return {
            "order_id": order_id, "merchant_id": "m_woo",
            "payment_status": "paid", "shopify_order_id": None,
            "payment_intent_id": "pi_woo", "total": "10.00", "currency": "USD",
        }
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.post(
            "/api/payments/webhooks/checkout",
            json=_checkout_payload("ORD_WOO", event_id="evt_woo"),
        )
