...
1. STRUCTURE — the PSP charge primitive (`capture_offsession`) has exactly ONE
   caller in the repo: the shared gate module `services/acp_offsession_payment`.
   A new call site (a second, gate-free charge path) fails the allowlist.
...
# The ONE module allowed to invoke the PSP charge primitive.
...
def _call_sites(symbol: str) -> set:
    """Repo-relative files (routes/ + services/) containing a CALL of symbol."""
    pattern = re.compile(rf"\b{re.escape(symbol)}\(")
    hits = set()
    for base in ("routes", "services"):
...
class TestSingleChargeChain:
    def test_capture_offsession_has_exactly_one_caller(self):
        assert _call_sites("capture_offsession") == {CHARGE_GATE_MODULE}, (
            "capture_offsession (the PSP charge primitive) may only be invoked "
            "by services/acp_offsession_payment.py — the shared gate module. A "
            "new call site is a charge path that skips the Tier-2 gate chain."
        )
