...
async def test_sync_projects_offer_under_real_seller(monkeypatch):
    ...
    # THE AUDIT BLIND SPOT (2026-07-27). This writer had the seed's domain in
    # hand — it puts it in offer_payload — but never wrote the source_domain
    # COLUMN, which is the one scripts/audit_offer_currency.py scans. 4,718 of
    # 18,809 live prod offers carried no source_domain as a result.
    assert params["source_domain"] == "x.test"


@pytest.mark.asyncio
async def test_source_domain_is_stored_verbatim_not_host_normalised(monkeypatch):
    """`www.` must SURVIVE. The audit and the currency backfill read
    `coalesce(offer.source_domain, seed.domain)` as ONE grouping key, so
    normalising on the write side but not on the seed side would split a single
    storefront across two report rows — and a split domain can fall under the
    per-domain `--min-offers` floor and stop being scanned at all. Real prod
    seeds include `www.jurlique.com`."""
    monkeypatch.setenv("EXTERNAL_OFFER_DUAL_WRITE_ENABLED", "1")
    seed = {
        "id": "s1", "external_product_id": "ext1",
        "price_amount": 19.99, "price_currency": "USD", "availability": "in_stock",
        "destination_url": "https://www.Jurlique.com/p", "canonical_url": None,
        "domain": "  WWW.Jurlique.com  ", "market": "US",
    }
    fake = FakeDB(seed=seed, mirror={"product_key": _REAL_PK, "merchant_id": _REAL_SELLER})
    monkeypatch.setattr(mod, "database", fake)

    await mod.sync_offer_for_seed("s1")
    assert fake.executed[0]["params"]["source_domain"] == "www.jurlique.com"


@pytest.mark.asyncio
async def test_blank_seed_domain_becomes_null_never_empty_string(monkeypatch):
    """An empty string would be a FALSE provenance signal: the audit's
    `nullif(btrim(source_domain),'')` reads it as absent, but a plain
    `source_domain IS NOT NULL` check anywhere else reads it as present."""
    monkeypatch.setenv("EXTERNAL_OFFER_DUAL_WRITE_ENABLED", "1")
    seed = {
        "id": "s1", "external_product_id": "ext1",
        "price_amount": 19.99, "price_currency": "USD", "availability": "in_stock",
        "destination_url": "https://x.test/p", "canonical_url": None,
        "domain": "   ", "market": "US",
    }
    fake = FakeDB(seed=seed, mirror={"product_key": _REAL_PK, "merchant_id": _REAL_SELLER})
    monkeypatch.setattr(mod, "database", fake)
...
@pytest.mark.asyncio
async def test_source_domain_upsert_is_fill_only(monkeypatch):
    """A re-project must not clobber a domain another pass resolved. The ON
    CONFLICT arm COALESCEs onto the STORED value rather than taking EXCLUDED."""
    monkeypatch.setenv("EXTERNAL_OFFER_DUAL_WRITE_ENABLED", "1")
    seed = {
        "id": "s1", "external_product_id": "ext1",
        "price_amount": 19.99, "price_currency": "USD", "availability": "in_stock",
    ...
    await mod.sync_offer_for_seed("s1")
    sql = " ".join(fake.executed[0]["sql"].split()).lower()
    assert (
