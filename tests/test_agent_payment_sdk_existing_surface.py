...
@pytest.mark.asyncio
async def test_agent_payments_finalizes_order_when_psp_already_succeeded(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """BUG-1 fix: when the PSP returns status='succeeded' synchronously, the order is finalized to PAID
    immediately via finalize_payment_success — NOT left at 'processing' waiting on a (possibly broken)
    webhook. Removes the stranded-charge dependency seen in the live test."""
    import mvp.events as mvp_events
    import mvp.governance as mvp_governance
    import routes.agent_payment_sdk as payment_module
    import services.psp_payment_finalizer as finalizer_module
    ...
    async def fake_create_payment_with_failover(*args: Any, **kwargs: Any):
        return True, SimpleNamespace(
            id="pi_succeeded_123",
            client_secret="pi_succeeded_123_secret",
            status="succeeded",
            amount=169,
            raw_response={},
        ), None, "stripe"
    ...
    async def fake_finalize_payment_success(order, **kwargs):
        finalized["order_id"] = order.get("order_id")
        finalized["psp"] = kwargs.get("psp")
        finalized["payment_reference"] = kwargs.get("payment_reference")
        finalized["source_event"] = kwargs.get("source_event")
        return {"applied": True, "order_id": order.get("order_id")}

    processing_updates = {"count": 0}

    async def fake_update_payment_info(*args: Any, **kwargs: Any):
        if kwargs.get("payment_status") == "processing":
            processing_updates["count"] += 1
        return True

    async def fake_mark_order_paid(order_id: str):
        return True

    async def fake_log_order_event(*args: Any, **kwargs: Any):
        return None
    ...
    monkeypatch.setattr(payment_module, "create_payment_with_failover", fake_create_payment_with_failover)
    monkeypatch.setattr(payment_module, "update_payment_info", fake_update_payment_info)
    # BUG-1 uses local imports inside create_payment; patch the SOURCE modules.
    monkeypatch.setattr(finalizer_module, "finalize_payment_success", fake_finalize_payment_success)
    monkeypatch.setattr(orders_module, "mark_order_paid", fake_mark_order_paid)
    monkeypatch.setattr(products_module, "log_order_event", fake_log_order_event)
    monkeypatch.setattr(database_obj, "fetch_one", fake_fetch_one)
    monkeypatch.setattr(database_obj, "execute", fake_execute)
    monkeypatch.setattr(mvp_events, "emit_best_effort", lambda **_: None)
    monkeypatch.setattr(mvp_governance.governance, "evaluate", lambda *_a, **_k: _Decision())
    ...
    # The order was finalized to PAID synchronously via the shared finalizer...
    assert finalized.get("order_id") == "ORD_SUCCEEDED_1"
    assert finalized.get("psp") == "stripe"
    assert finalized.get("payment_reference") == "pi_succeeded_123"
    assert finalized.get("source_event") == "payment_confirmed_sync"
    # ...and it was NOT left at 'processing' (no webhook dependency for a succeeded charge).
    assert processing_updates["count"] == 0
