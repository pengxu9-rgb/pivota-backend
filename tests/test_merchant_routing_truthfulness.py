...
@pytest.mark.asyncio
async def test_update_merchant_routing_config_respects_explicit_subset(
    ...
    async def fake_get_active_merchant_psps(merchant_id: str):
        ...
        return [
            {"provider": "stripe", "psp_id": "psp_stripe_1"},
            {"provider": "adyen", "psp_id": "psp_adyen_1"},
            {"provider": "checkout", "psp_id": "psp_checkout_1"},
        ]
    ...
    assert response["status"] == "success"
    assert response["data"]["psp_priority"] == [{"psp": "adyen", "priority": 1}]
    assert response["data"]["max_retries"] == 0
    assert json.loads(executed["psp_priority"]) == [{"psp": "adyen", "priority": 1}]
