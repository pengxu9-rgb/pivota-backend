...
async def _order(order_id: str, *, total: str, status: str, merchant: str = MERCHANT,
                 days_ago: int = 1, email: str = "buyer@example.com") -> None:
...
async def test_finance_overview_sums_the_order_total_rather_than_answering_zero():
    ...
    await _order("revcol-fin-1", total="100.00", status="completed")
    await _order("revcol-fin-2", total="40.00", status="pending")
    ...
    # gross = every order added in the 30-day window; net = the
    # completed/delivered ones only. Both are the SUM(total) the fix introduced.
...
async def test_payment_analytics_reports_the_amount_per_status():
    ...
    await _order("revcol-an-1", total="25.50", status="completed")
    await _order("revcol-an-2", total="74.50", status="completed")
    ...
    assert delta("completed") == (100.0, 2)
