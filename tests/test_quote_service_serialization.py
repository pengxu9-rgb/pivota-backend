...
async def test_preview_quote_returns_nested_discount_evidence_json_safe(monkeypatch):
    ...
    # Promotions lane deleted (ADR-022): the quote payload must no longer carry
    # a store_discount_evidence key at all — absence pins the deletion.
    assert "store_discount_evidence" not in result
    assert "store_discount_evidence" not in captured_quote["snapshot_json"]
