...
def test_a_fully_skipped_subtree_is_rejected(tmp_path):
    ...
    proc = _run(tmp_path, _xml(services=3249, readiness=133, skip_readiness=True))
...
def test_a_healthy_run_still_passes(tmp_path):
    ...
    proc = _run(tmp_path, _xml(services=3249, readiness=133))
...
def test_partial_skips_are_subtracted(tmp_path):
    ...
    proc = _run(tmp_path, _xml(services=3249, readiness=133, skip_readiness=False,
...
def test_the_global_floor_still_bites(tmp_path):
    ...
    proc = _run(tmp_path, _xml(services=3249, readiness=133, other=100))
    assert proc.returncode != 0
    ...
    out = proc.stdout + proc.stderr
    assert "only 3482 tests executed" in out, out
...
def test_the_healthy_fixture_is_healthy_by_the_CURRENT_floor():
    ...
    root = ET.fromstring(_xml(services=3249, readiness=133))
