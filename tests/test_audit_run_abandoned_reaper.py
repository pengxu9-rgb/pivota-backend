...
async def test_fails_abandoned_runs_both_orphan_classes(sqlite_db):
    ...
    # 1. Abandoned wedge run: never leased (claimed_until NULL), old.
    await _insert(
    ...
    n = await fail_abandoned_runs(ttl_seconds=3600)
    assert n == 2
...
async def test_preserves_existing_error_message(sqlite_db):
    ...
    assert await fail_abandoned_runs(ttl_seconds=3600) == 1
    row = await sqlite_db.fetch_one(
