...
def test_guard_flag_off_never_queries(monkeypatch):
    monkeypatch.delenv("ENABLE_AUDIT_BRAND_FRAGMENTATION_GUARD", raising=False)

    def _should_not_run(query, values):
        raise AssertionError("guard must not query when flag is off")
...
def test_guard_skips_mint_on_brand_conflict(monkeypatch):
    monkeypatch.setenv("ENABLE_AUDIT_BRAND_FRAGMENTATION_GUARD", "1")
