...
async def get_merchant_stores(
    ...
    """Return connected stores from both new merchant_stores table and legacy integrations."""
    stores: List[Dict[str, Any]] = []
    seen_ids = set()

    try:
        # 1. Primary source: merchant_stores (Phase 6+)
        new_query = """
            SELECT 
                store_id,
                platform,
        ...
        """
        new_rows = await database.fetch_all(new_query, {"merchant_id": merchant_id})
        ...
        for row in new_rows:
            ...
            stores.append({
                ...
                "name": r.get("name") or r.get("domain") or "Store",
                "domain": r.get("domain"),
                "status": r.get("status"),
                "connected_at": r.get("connected_at").isoformat() if r.get("connected_at") else None,
                "last_sync": r.get("last_sync").isoformat() if r.get("last_sync") else None,
                "product_count": r.get("product_count") or 0
            })

        # 2. Legacy fallback: merchant_store_integrations
        legacy_query = """
            SELECT 
                integration_id,
                platform,
            ...
            GROUP BY integration_id, platform, store_name, domain, status, created_at
            ORDER BY created_at DESC NULLS LAST
        """
        legacy_rows = await database.fetch_all(legacy_query, {"merchant_id": merchant_id})

        for row in legacy_rows:
            r = dict(row)
            store_id = r.get("integration_id")
            if store_id in seen_ids:
                continue
            stores.append({
                "id": store_id,
                "platform": r.get("platform"),
                "name": r.get("store_name") or r.get("domain") or "Store",
                "domain": r.get("domain"),
                "status": r.get("status"),
...
async def get_merchant_psps(
    ...
    """Return connected PSPs using the new merchant_psps table (with legacy fallback)."""
    ...
    try:
        query = """
            ...
            GROUP BY mp.psp_id, mp.provider, mp.name, mp.status, mp.connected_at
            ORDER BY mp.connected_at DESC NULLS LAST
        """
        ...
        for row in rows:
            ...
            psps.append({
                "id": r.get("psp_id"),
                "name": r.get("name") or r.get("provider", "PSP").upper(),
        ...
        # Legacy fallback from merchant_onboarding
        if not rows:
            legacy = await database.fetch_one(
                "SELECT psp_connected, psp_type FROM merchant_onboarding WHERE merchant_id = :merchant_id",
                {"merchant_id": merchant_id}
            )
            if legacy and legacy.get("psp_connected"):
                psps.append({
                    "id": f"legacy_{legacy.get('psp_type')}",
                    "name": legacy.get("psp_type", "PSP").upper(),
                    "provider": legacy.get("psp_type"),
                    "status": "active",
                    "connected_at": None,
                    "transaction_count": 0,
                    "total_volume": 0.0
