...
async def get_merchant_visibility_tracking(
    ...
    `subject_type` picks which run kind to trend: 'merchant' (catalog audits,
    default) or 'merchant_url' (the URL-audit wedge) — the two kinds are separate
    series and must never be mixed on one chart."""
    if limit <= 0 or limit > 50:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="limit must be between 1 and 50",
    ...
    return {"merchant_id": merchant_id, "subject_type": subject_type, **series}
